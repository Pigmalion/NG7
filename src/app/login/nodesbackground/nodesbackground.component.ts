import { Component, OnInit } from '@angular/core';

@Component({
  selector: 'app-nodesbackground',
  templateUrl: './nodesbackground.component.html',
  styleUrls: ['./nodesbackground.component.css']
})
export class NodesbackgroundComponent implements OnInit {

  constructor() { }

  ngOnInit() {

    const Splace = (function () {
      function Splace() {
        this.canvas = document.createElement('canvas');
        this.ctx = this.canvas.getContext('2d');
        this.particles = [];
        this.ratio = window.innerHeight < 400 ? 0.6 : 1;
        this.r = 120;
        this.counter = 0;
      }
      Splace.prototype.init = function () {
        this.createElement();
        this.loop();
      };
      Splace.prototype.createElement = function () {
        const scale = this.ratio;
        this.canvas.width = window.innerWidth;
        this.canvas.height = window.innerHeight;
        this.canvas.style.width = '100%';
        this.canvas.style.position = 'absolute';
        this.canvas.style.top = '3%';
        this.canvas.style.left = '0';
        this.canvas.style.zIndex = '-1';

        this.canvas.style.background = 'rgb(0, 0, 0)';
        this.ctx.transform(scale, 0, 0, -scale, this.canvas.width / 2, this.canvas.height / 2);
        document.body.appendChild(this.canvas);
        for (let i = 0; i < 450; i++) {
          this.createParticle();
        }
      };
      Splace.prototype.createParticle = function () {
        this.particles.push({
          color: Math.random() > 0.5 ? 'rgba(70, 150, 180, 1)' : 'rgba(30, 190, 255, 0.4)',
          radius: Math.random() * 5,
          x: Math.cos(Math.random() * 7 + Math.PI) * this.r,
          y: Math.sin(Math.random() * 7 + Math.PI) * this.r,
          ring: Math.random() * this.r * 3,
          move: ((Math.random() * 4) + 1) / 500,
          random: Math.random() * 7
        });
      };
      Splace.prototype.moveParticle = function (p) {
        p.ring = Math.max(p.ring - 1, this.r);
        p.random += p.move;
        p.x = Math.cos(p.random + Math.PI) * p.ring;
        p.y = Math.sin(p.random + Math.PI) * p.ring;
      };
      Splace.prototype.resetParticle = function (p) {
        p.ring = Math.random() * this.r * 3;
        p.radius = Math.random() * 5;
      };
      Splace.prototype.disappear = function (p) {
        if (p.radius < 0.8) {
          this.resetParticle(p);
        }
        p.radius *= 0.994;
      };
      Splace.prototype.draw = function (p) {
        this.ctx.beginPath();
        this.ctx.fillStyle = p.color;
        this.ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
        this.ctx.fill();
      };
      Splace.prototype.loop = function () {
        this.ctx.clearRect(-this.canvas.width , -this.canvas.height, this.canvas.width * 2, this.canvas.height * 2);
        if (this.counter < this.particles.length) {
          this.counter++;
        }
        // this.ctx.shadowBlur = 20;
        // this.ctx.shadowColor = "#fff";
        for (let i = 0; i < this.counter; i++) {
          this.disappear(this.particles[i]);
          this.moveParticle(this.particles[i]);
          this.draw(this.particles[i]);
        }
        requestAnimationFrame(this.loop.bind(this));
      };
      return Splace;
    })();
    window.onload = function () {
      const space = new Splace();
      space.init();
    };

  }

}
